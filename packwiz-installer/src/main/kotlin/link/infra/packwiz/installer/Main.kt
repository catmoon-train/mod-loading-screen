...
class Main(args: Array<String>) {
	...
	private fun startup(args: Array<String>) {
			...
			if (guiEnabled) {
				EventQueue.invokeAndWait {
					...
					JOptionPane.showMessageDialog(null, "Failed to parse command line arguments: $e",
						"模组包更新器", JOptionPane.ERROR_MESSAGE)
				}
			}
			exitProcess(1)
		}
		...
		if (unparsedArgs.size > 1) {
			ui.showErrorAndExit("参数过多！")
		} else if (unparsedArgs.isEmpty()) {
			ui.showErrorAndExit("必须提供用于安装的 pack.toml 地址！")
		}
		...
		val packFile = when {
			// HTTP(s) URLs
			Regex("^https?://", RegexOption.IGNORE_CASE).containsMatchIn(packFileRaw) -> ui.wrap("pack 文件 HTTP/HTTPS 地址无效：$packFileRaw") {
			...
			Regex("^file:", RegexOption.IGNORE_CASE).containsMatchIn(packFileRaw) -> {
				ui.wrap("解析 pack 文件路径失败：$packFileRaw") {
					val path = Paths.get(URI(packFileRaw)).toOkioPath()
					PackwizFilePath(path.parent ?: ui.showErrorAndExit("pack 文件路径无效：$packFileRaw"), path.name)
				}
			}
			// Other URIs (unsupported)
			Regex("^[a-z][a-z\\d+\\-.]*://", RegexOption.IGNORE_CASE).containsMatchIn(packFileRaw) -> ui.showErrorAndExit("不支持的 pack 文件协议：$packFileRaw")
			// None of the above matches -> interpret as file path
			else -> PackwizFilePath(packFileRaw.toPath().parent ?: ui.showErrorAndExit("pack 文件路径无效：$packFileRaw"), packFileRaw.toPath().name)
		}
		val side = cmd.getOptionValue("side")?.let {
			Side.from(it) ?: ui.showErrorAndExit("未知的 side 参数：$it")
		} ?: Side.CLIENT
		val packFolder = ui.wrap("pack 目录路径无效") {
			cmd.getOptionValue("pack-folder")?.let{ PackwizFilePath(it.toPath()) } ?: PackwizFilePath(".".toPath())
		}
		val multimcFolder = ui.wrap("MultiMC 目录路径无效") {
			cmd.getOptionValue("multimc-folder")?.let{ PackwizFilePath(it.toPath()) } ?: PackwizFilePath("..".toPath())
		}
		val manifestFile = ui.wrap("manifest 文件路径无效") {
			packFolder / (cmd.getOptionValue("meta-file") ?: "packwiz.json")
		}
		val timeout = ui.wrap("超时参数无效") {
		...
		try {
			...
			ui.showErrorAndExit("更新流程执行失败", e)
		}
		println("更新流程执行完成！")
		ui.dispose()
	}
