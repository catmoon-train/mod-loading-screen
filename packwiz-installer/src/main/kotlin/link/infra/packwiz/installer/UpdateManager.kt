...
class UpdateManager internal constructor(private val opts: Options, val ui: IUserInterface) {
	...
	private fun start() {
		...
		ui.submitProgress(InstallProgress("正在加载本地清单文件..."))
		...
		val manifest = try {
			...
			ui.showErrorAndExit("读取本地清单文件失败，请尝试删除 ${opts.manifestFile}", e)
		}
		...
		ui.submitProgress(InstallProgress("正在加载 pack 文件..."))
		...
		ui.submitProgress(InstallProgress("正在读取 MultiMC 配置..."))
		try {
		...
		ui.submitProgress(InstallProgress("正在检查本地文件..."))
		...
		if (!invalidateAll) {
			...
			if (manifest.packFileHash?.let { it == packFileSource.hash } == true && invalidatedUris.isEmpty()) {
				// todo: --force?
				ui.submitProgress(InstallProgress("模组包已是最新版本。", 1, 1))
		...
		try {
			...
			ui.showErrorAndExit("处理索引文件失败", e1)
		}
	...
	private fun processIndex(indexUri: PackwizPath<*>, indexHash: Hash<*>, hashFormat: HashFormat<*>, manifest: ManifestFile, invalidatedFiles: List<PackwizFilePath>, invalidateAll: Boolean, clientHolder: ClientHolder) {
		if (!invalidateAll) {
			if (manifest.indexFileHash == indexHash && invalidatedFiles.isEmpty()) {
				ui.submitProgress(InstallProgress("模组包文件已是最新版本。", 1, 1))
				if (manifest.cachedFiles.any { it.value.isOptional }) {
					ui.awaitOptionalButton(false, opts.timeout)
				}
				if (!ui.optionsButtonPressed) {
		...
		if (indexHash != indexFileSource.hash) {
			ui.showErrorAndExit("索引文件哈希校验失败！请联系整合包作者执行一次 packwiz refresh。")
		}
		...
		ui.submitProgress(InstallProgress("正在比较新文件..."))
		...
		if (optionTasks.isNotEmpty() && !optionsChanged) {
			if (!ui.optionsButtonPressed) {
				...
				ui.submitProgress(InstallProgress("是否重新配置可选模组？", 0,1))
		...
		for (i in tasks.indices) {
			val task: DownloadTask = try {
				...
				ui.showErrorAndExit("下载任务执行被中断", e)
			} catch (e: ExecutionException) {
				ui.showErrorAndExit("执行下载任务失败", e)
			}
			...
			val progress = if (exDetails != null) {
				"下载失败 ${exDetails.name}: ${exDetails.exception.message}"
			} else {
				when (task.completionStatus) {
					DownloadTask.CompletionStatus.INCOMPLETE -> "${task.name} 等待中（通常不应出现）"
					DownloadTask.CompletionStatus.DOWNLOADED -> "已下载 ${task.name}"
					DownloadTask.CompletionStatus.ALREADY_EXISTS_CACHED -> "${task.name} 已存在（缓存命中）"
					DownloadTask.CompletionStatus.ALREADY_EXISTS_VALIDATED -> "${task.name} 已存在（校验通过）"
					DownloadTask.CompletionStatus.SKIPPED_DISABLED -> "已跳过 ${task.name}（已禁用）"
					DownloadTask.CompletionStatus.SKIPPED_WRONG_SIDE -> "已跳过 ${task.name}（不匹配当前 side）"
					DownloadTask.CompletionStatus.DELETED_DISABLED -> "已删除 ${task.name}（已禁用）"
					DownloadTask.CompletionStatus.DELETED_WRONG_SIDE -> "已删除 ${task.name}（不匹配当前 side）"
				}
			}
	...
	private fun validateAndResolve(nonFailedFirstTasks: List<DownloadTask>, clientHolder: ClientHolder): ResolveResult {
		ui.submitProgress(InstallProgress("正在校验已有文件..."))
		...
		if (cfFiles.isNotEmpty()) {
			ui.submitProgress(InstallProgress("正在解析 CurseForge 元数据..."))
